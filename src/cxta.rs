...
impl AssetDriver for CxtaDriver {
    ...
    fn pack(&self, folder: &Path) -> io::Result<Vec<u8>> {
                ...
                let img = ImageReader::open(&path)
                    ...
                    .map_err(|e| {
                        io::Error::new(
                            ...
                            format!("Failed to decode image {:?}: {}", path, e),
                        )
                    })?
                    .into_rgba8();
        ...
        for (name, img) in &successful_images {
            if img.dimensions() != (first_w, first_h) {
                return Err(io::Error::new(
                    io::ErrorKind::InvalidData,
                    format!(
                        "Image '{}' has different dimensions ({}x{}) than the first one ({}x{})",
                        name,
                        img.width(),
                        img.height(),
                        first_w,
        ...
        if atlas_w > MAX_ATLAS_DIM || atlas_h > MAX_ATLAS_DIM {
            return Err(io::Error::new(
                io::ErrorKind::InvalidData,
                "Atlas dimensions exceed MAX_ATLAS_DIM",
            ));
        }
        ...
        {
            let mut encoder =
                png::Encoder::new(&mut png_data, atlas_image.width(), atlas_image.height());
    ...
    fn unpack(&self, data: &[u8], folder: &Path) -> io::Result<()> {
        ...
        if offset + png_len > data.len() {
            return Err(io::Error::new(
                io::ErrorKind::InvalidData,
                "Unexpected end of CXTA PNG data",
            ));
        }
        let png_data = &data[offset..offset + png_len];
        offset += png_len;

        let mut reader = ImageReader::new(io::Cursor::new(png_data)).with_guessed_format()?;
        reader.no_limits();
        ...
        for _ in 0..item_count {
            if offset + 16 > data.len() {
                return Err(io::Error::new(
                    io::ErrorKind::InvalidData,
                    "Unexpected end of CXTA item data",
                ));
            }
            ...
            img.save(folder.join(&file_name))
                .map_err(|e| io::Error::new(io::ErrorKind::Other, e))?;

            images_toml.push(format!(
                "{{ name = \"{}\", file = \"{}\" }}",
                name, file_name
            ));
        }
