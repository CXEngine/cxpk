...
impl AssetDriver for CxsiDriver {
    ...
    fn pack(&self, folder: &Path) -> io::Result<Vec<u8>> {
        ...
        for v in variants {
            let file = v.get("file").and_then(|s| s.as_str()).ok_or_else(|| {
                io::Error::new(io::ErrorKind::InvalidData, "variant.file missing")
            })?;
    ...
    fn unpack(&self, data: &[u8], folder: &Path) -> io::Result<()> {
        ...
        for i in 0..variant_count {
            ...
            if offset + png_len > data.len() {
                return Err(io::Error::new(
                    io::ErrorKind::InvalidData,
                    "Unexpected end of CXSI PNG data",
                ));
            }
